...
**Вопросы Claude Code и Codex — на Mac, iPhone и Apple Watch.**

Pingvi показывает, какой агент ждёт вашего ответа, уведомляет о завершении работы
и позволяет отвечать на поддерживаемые вопросы прямо из приложения. На iPhone можно
прочитать переписку, продолжить готовый диалог или создать новый; данные передаются
напрямую через локальную сеть. На Mac диалоги можно объединять в рабочие пространства
и дополнять ответы ссылками на локальные файлы и скриншоты.
...
![iOS 18+](https://img.shields.io/badge/iOS-18%2B-1677ff?style=flat-square&logo=apple&logoColor=white)
![watchOS 11+](https://img.shields.io/badge/watchOS-11%2B-111827?style=flat-square&logo=apple&logoColor=white)
...
4. Чтобы отвечать с iPhone или Apple Watch, свяжите личное устройство с Mac по QR-коду и оставьте их в одной локальной сети.
...
| **Локальный мобильный прототип** | Получать очередь и отвечать с личных iPhone и Apple Watch в одной локальной сети. |
...
### iPhone и Apple Watch — локальный прототип

В **Настройки → Подключения** можно создать QR-код и связать один личный iPhone с Mac. Вопросы передаются по зашифрованному локальному соединению; Apple Watch получает очередь через сопряжённый iPhone. Мобильный ответ проходит через тот же механизм проверки token и доставки, что и ответ в Mac-клиенте.

iOS-клиент использует адаптивные стеклянные карточки и нижнюю навигацию. В мобильных настройках можно выбрать системную, светлую или тёмную тему и одну из пяти иконок Pingvi; выбранная иконка устанавливается на домашний экран через системный механизм iOS. Вкладка «Диалоги» показывает все доступные сессии: с iPhone можно открыть готовый результат, прочитать историю, продолжить свободный диалог и создать новую сессию в известном Mac-проекте.

<p align="center">
  <img src="docs/media/iphone-queue.png" width="45%" alt="Очередь вопросов и готовых результатов Pingvi на iPhone">
  <img src="docs/media/iphone-dialogs.png" width="45%" alt="Список диалогов Claude Code и Codex в Pingvi на iPhone">
</p>
<p align="center">
  <img src="docs/media/iphone-conversation.png" width="45%" alt="Переписка с агентом и поле быстрого ответа в Pingvi на iPhone">
  <img src="docs/media/iphone-appearance.png" width="45%" alt="Выбор темы и иконки Pingvi на iPhone">
</p>

<p align="center"><sub>Настоящий интерфейс iPhone-приложения с вымышленными проектами и перепиской; личные данные не использовались.</sub></p>

Для прототипа не нужны собственный сервер, APNs или платная Apple Developer Program. Поэтому есть намеренные ограничения: мобильные приложения устанавливаются из Xcode через бесплатную Personal Team и требуют повторной установки каждые 7 дней; после приостановки iOS-приложения доставка по LAN не гарантируется; уведомление появляется на iPhone либо Watch по системным правилам Apple. Ответ отправляется только при живом соединении с Mac, без офлайн-очереди.

[Инструкция по сборке и установке мобильного клиента](Mobile/README.md).

## Установка

**0.7.0 beta 11**: добавлено локальное подключение личного iPhone и Apple Watch. На iPhone доступны очередь вопросов, быстрые ответы, готовые результаты, история диалогов, продолжение и создание сессий, темы и сменные иконки. Также исправлена отправка вариантов ответа: мобильный клиент передаёт стабильный идентификатор варианта, а после ошибки позволяет повторить отправку. Mac-клиент сохраняет исправления производительности beta 10.

**[Скачать Pingvi 0.7.0 beta 11 — DMG для Apple Silicon](https://github.com/leshchinskaya/pingvi/releases/download/v0.7.0-beta.11/Pingvi-0.7.0-beta.11-arm64-adhoc.dmg)**
...
Для локального тестирования создаётся `dist/Pingvi-0.7.0-beta.11-arm64-local.dmg` и SHA-256 рядом. Для распространяемой сборки используйте `bash tools/package-dmg.sh --release`: нужны `SIGNING_IDENTITY` (Developer ID Application) и `NOTARY_PROFILE`. Скрипт требует notarization и проверку Gatekeeper.
...
- Если сам DMG не открывается, скачайте его заново вместе с `.sha256` из того же релиза и проверьте `shasum -a 256 -c Pingvi-0.7.0-beta.11-arm64-adhoc.dmg.sha256` в папке загрузки.
