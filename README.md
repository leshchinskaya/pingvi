...
Агенту нужно уточнение — Pingvi показывает плавающую карточку поверх других окон. Сразу видно, какая сессия и проект ждут ответа: выберите вариант или напишите своё уточнение и нажмите **самолётик рядом с полем ввода**. Enter в поле отправляет ответ, Shift+Enter добавляет новую строку. Для поддерживаемых вопросов ответ уйдёт в исходную сессию ИИ.

Для распознанных вопросов Codex карточка показывает текущий вопрос с пояснениями, без предыдущего вопроса, вашего прошлого ответа и строки ввода терминала. В подтверждениях команд варианты ответа отображаются отдельно от текста действия.
...
### Продолжайте с нового

В чате граница **«Новое»** отделяет сообщения после вашего последнего ответа. При возвращении сохраняется место чтения; нажатие на уведомление открывает новое. Обновления не прокручивают переписку автоматически. Совпадающий контекст терминала можно развернуть в блоке **«До вашего ответа»**; если границу нельзя определить, текст показывается полностью.

В вопросах выбор варианта и поле **«Дополнить ответ…»** разделены. После отправки Pingvi ждёт изменения состояния агента и переходит к следующему вопросу. При неизвестном результате отправки или после вашего перехода в другой диалог автоматического переключения нет.
...
**Enter** отправляет сообщение, **Shift+Enter** добавляет новую строку. Также можно нажать самолётик рядом с полем ввода или **⌘Enter**. Если агент занят, текст остаётся черновиком: автоматической отправки после завершения нет. Запросы разрешений показываются отдельно.
