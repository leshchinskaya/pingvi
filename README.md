...
**0.7.0 beta 9**: исправления доставки уведомлений, диагностика разрешений и инструкция установки. Также доступны поиск по переписке, контекст перед вопросом, личные заметки, скриншоты через ⌘V, шаблоны ответов и обзор готовых результатов. Скриншоты выше показывают интерфейс beta 5.

**[Скачать Pingvi 0.7.0 beta 9 — DMG для Apple Silicon](https://github.com/leshchinskaya/pingvi/releases/download/v0.7.0-beta.9/Pingvi-0.7.0-beta.9-arm64-adhoc.dmg)**
...
Для локального тестирования создаётся `dist/Pingvi-0.7.0-beta.9-arm64-local.dmg` и SHA-256 рядом. Для распространяемой сборки используйте `bash tools/package-dmg.sh --release`: нужны `SIGNING_IDENTITY` (Developer ID Application) и `NOTARY_PROFILE`. Скрипт требует notarization и проверку Gatekeeper.
...
- Если сам DMG не открывается, скачайте его заново вместе с `.sha256` из того же релиза и проверьте `shasum -a 256 -c Pingvi-0.7.0-beta.9-arm64-adhoc.dmg.sha256` в папке загрузки.
